...
const login = async (req, res) => {
  ...
  try {
    const existingEmployee = await EmployeeModel.findOne({ email }).populate('role', 'typeName');
    ...
    const token = jwt.sign({ id: existingEmployee.id, isAdmin: existingEmployee.role.typeName === 'Admin' }, process.env.JWT_PASS, { expiresIn: '1h' });
